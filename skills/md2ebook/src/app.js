...
let searchIndex = [];   // [{page, text}] — 페이지 분할과 무관한 전체 노드 검색 인덱스
let srchQuery = '', srchHits = [], srchHI = 0, srchMarks = [], srchRenderedPage = -1, searchBarOpen = false;
let current = 0;
...
function paginate(){
  ...
  let curNodes = [];
  const nodePage = new Map();   // 분할 후 각 노드 → 소속 페이지 인덱스 (검색 점프용)
  const flush = () => {
    ...
    curNodes.forEach(n => nodePage.set(n, pages.length));
  ...
  // 검색 인덱스: 분할된 노드 단위로 텍스트 + 소속 페이지 (브라우저 기본 Ctrl+F 가 못 보는 다른 페이지까지 커버)
  searchIndex = nodes.map(n => ({ page: nodePage.get(n) ?? 0, text: (n.textContent || '').toLowerCase() }));
}
...
window.addEventListener('keydown', e => {
  ...
  if (e.key === 'Escape'){ if (searchBarOpen) closeSearch(); else closeSheets(); return; }
  if (/^(INPUT|TEXTAREA)$/.test((document.activeElement||{}).tagName || '')) return;  // 입력 중엔 페이지 네비 무시
...
/* =========================================================================
   6.5) 리더 내 검색 (Ctrl+F) — srcNodes 전체를 검색해 페이지 분할을 넘어 찾는다
   ========================================================================= */
// pad 안 텍스트에서 q 출현부를 <mark> 로 감싸고, 그 mark 들을 문서 순서로 반환. (svg·기존 mark·복사버튼 제외)
function highlightPad(pad, q){
  const marks = [];
  if (!q) return marks;
  const walker = document.createTreeWalker(pad, NodeFilter.SHOW_TEXT, {
    acceptNode: n => (n.nodeValue && n.nodeValue.toLowerCase().includes(q)
      && !(n.parentElement && n.parentElement.closest('svg, mark, .copy-btn')))
      ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
  });
  const tns = []; let t; while ((t = walker.nextNode())) tns.push(t);
  tns.forEach(tn => {
    const text = tn.nodeValue, lower = text.toLowerCase();
...
function clearSearchMarks(pad){
  pad.querySelectorAll('mark.search-hit').forEach(m => m.replaceWith(document.createTextNode(m.textContent)));
  pad.normalize();
}
function updateSearchCount(){
  $('#searchCount').textContent = srchHits.length ? (srchHI+1)+' / '+srchHits.length : (srchQuery ? '없음' : '');
}
function doSearch(raw){
  const q = (raw || '').trim().toLowerCase();
  srchQuery = q; srchHI = 0; srchRenderedPage = -1; srchHits = [];
  clearSearchMarks(padBelow); srchMarks = [];
  if (!q){ updateSearchCount(); return; }
  if (settings.flip === 'scroll'){
    // 스크롤 모드: padBelow 에 전체가 이미 있으므로 통째로 하이라이트하고 마크 단위로 이동.
    srchMarks = highlightPad(padBelow, q);
    srchHits = srchMarks.map((_, i) => ({ scroll: true, i }));
  } else {
    // 페이지 모드: 전체 인덱스에서 출현부를 모아 {페이지, 그 페이지 내 순번} 으로 보관.
    const perPage = {};
    searchIndex.forEach(e => {
      let idx = e.text.indexOf(q);
      while (idx >= 0){ perPage[e.page] = perPage[e.page] || 0; srchHits.push({ page: e.page, m: perPage[e.page]++ }); idx = e.text.indexOf(q, idx + q.length); }
    });
  }
  if (srchHits.length) showHit(0); else updateSearchCount();
}
function showHit(hi){
  if (!srchHits.length){ updateSearchCount(); return; }
  srchHI = (hi % srchHits.length + srchHits.length) % srchHits.length;
  const h = srchHits[srchHI];
  let mk;
  if (h.scroll){ mk = srchMarks[h.i]; }
  else {
    if (h.page !== srchRenderedPage){
      current = clamp(h.page); renderBase(); updateChrome();
      clearSearchMarks(padBelow); srchMarks = highlightPad(padBelow, srchQuery); srchRenderedPage = h.page;
    }
    mk = srchMarks[h.m];
  }
  padBelow.querySelectorAll('mark.search-hit.active').forEach(m => m.classList.remove('active'));
  if (mk){ mk.classList.add('active'); mk.scrollIntoView({ block: 'center' }); }
  updateSearchCount();
}
function nextMatch(){ showHit(srchHI + 1); }
function prevMatch(){ showHit(srchHI - 1); }
function openSearch(){
  searchBarOpen = true; $('#searchBar').hidden = false;
  const inp = $('#searchInput'); inp.focus(); inp.select();
  if (inp.value.trim()) doSearch(inp.value);
}
function closeSearch(){
  searchBarOpen = false; $('#searchBar').hidden = true;
  clearSearchMarks(padBelow); srchHits = []; srchMarks = []; srchQuery = '';
}
...
function reflow(keepId){
  ...
  buildToc();
  updateChrome();
  if (searchBarOpen && srchQuery) doSearch($('#searchInput').value);   // 재분할 후 검색 결과·하이라이트 갱신
}
