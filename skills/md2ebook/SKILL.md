...
- **인터랙션**: 코드블록 우상단 「복사」 버튼 · 이미지 탭하면 라이트박스 확대 · 체크박스 토글(상태는 문서별 localStorage 저장, 새로고침·페이지 이동에도 유지).
...
지원: `#`~`######` 헤딩(H1~H6), `**굵게**`, `*기울임*`, `~~취소선~~`, 인라인 코드, 코드펜스, `> 인용`, `| 표 |`, 목록(`-` 비순서 · `1.` 번호 · 들여쓰기 중첩 · `- [ ]` 체크박스), `[텍스트](url)` 링크, `![대체](src)` 이미지, `---`, 그리고 `[사실]` `[중요]` 식 라벨 칩(아무 단어나 가능 — 위 "라벨 칩" 참조).
