...
- 리더에서 체크박스를 클릭하면 원본 `.md` 파일에 즉시 반영됩니다
- `.md` 링크를 클릭하면 VS Code 안에서 바로 열립니다

## 🚀 시작하기

1. `.md` 파일을 열고 편집 화면에서 우클릭
2. **"Ebook Reader: Open Preview"** 선택
   또는 `Ctrl+Shift+P` → `Ebook Reader` 입력
...
- 🏷️ **Color labels** — Use `` `[word]` `` for inline highlighting
  - Reserved words like `[important]` · `[TODO]` · `[warning]` get fixed colors
  - Any other word gets a color auto-assigned

## 🔗 Live sync

- Clicking a checkbox in the reader saves it back to the original `.md` file immediately
- Clicking a `.md` link opens it directly inside VS Code

## 🚀 Getting started

1. Open any `.md` file and right-click in the editor
2. Select **"Ebook Reader: Open Preview"**
   or press `Ctrl+Shift+P` → type `Ebook Reader`
