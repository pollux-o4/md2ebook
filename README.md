...
**마크다운 한 편을, 페이지 넘기는 한 장의 책으로.**

쓰던 `.md` 를 외부 의존성 0의 **오프라인 단일 HTML 책 리더**로 바꿔 주는 [Agent Skill](https://github.com/vercel-labs/skills).
CDN도 번들러도 런타임도 없이 **파일 하나** — 폰에서도 그냥 열린다. 내용과 디자인이 분리돼 있어, 글만 갈아끼우면 그대로 책이 된다.

> 소개 페이지: [`intro.html`](./intro.html) · 라이브로 넘겨보기: [`README.html`](./README.html) · [`test/test.html`](./test/test.html)
...
- **공유가 쉽다** — 결과물이 파일 하나. 메신저로 보내든 USB에 담든, 받는 사람은 더블클릭만 하면 책처럼 읽는다. 설치·계정·네트워크 전부 불필요.
- **오프라인에서 산다** — CDN·웹폰트·외부 스크립트가 0이라 비행기에서도, 폐쇄망에서도 똑같이 열린다. 링크가 죽을 일도 없다.
- **읽기 경험이 진짜 책 같다** — 화면 단위 페이지네이션, 넘김 애니메이션, 목차·진행바·몰입모드까지. 긴 글도 끝까지 읽힌다.
- **내 글은 그대로** — 내용과 디자인이 분리돼 있어, 다음 글도 마크다운만 바꿔 끼우면 같은 톤의 책이 나온다.
...
- **근거가 한눈에** — `[사실]` `[추정]` `[의견]` `[모름]` 라벨을 색칩으로. 아무 단어나 라벨이 되고, 같은 단어는 어디서나 같은 색이다.
