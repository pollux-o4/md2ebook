...
`[참고]` 자세한 옵션과 구성은 [README](https://pollux-o4.github.io/md2ebook/README.html)에 정리돼 있어요. 소개가 궁금하시면 [소개 페이지](https://pollux-o4.github.io/md2ebook/intro.html)부터 둘러보셔도 좋아요.
...
repo에서 바로 쓸 땐 `python skills/md2ebook/build.py 내문서.md 내문서.html`처럼 경로를 붙여 줘요.
...
- [ ] 근거가 필요한 문장에 색칩 달아 두기
- [ ] 표지 느낌을 줄 첫 문단 다듬기

`[중요]` 챕터는 H1이나 H2에서 나뉘어요. 글을 여러 꼭지로 보여주고 싶다면 그 경계마다 H1이나 H2를 두세요. 한 챕터가 화면 높이를 넘으면 리더가 알아서 다음 장으로 나눠 줘요.

## 책처럼 넘겨 읽기

리더를 열면 글이 화면 단위 페이지로 나뉘어요. 마음에 드는 방식으로 장을 넘기면 되는데, 네 가지 넘김이 준비돼 있어요.
...
여기서부터는 있으면 좋은 부가기능이에요. 먼저 색칩부터 볼게요.
...
리더에서는 `[중요]` `[디자인]` `[리뷰]` 처럼 또렷한 색칩으로 떠요. 대괄호만 쓰면 안 되고, 그 바깥을 백틱으로 한 번 더 감싸야 한다는 점만 기억하면 돼요.

`[사실]` `[추정]` `[의견]` `[모름]` 같은 의미 라벨, `[중요]` `[TODO]` 처럼 자주 쓰는 말은 색이 미리 정해져 있고, 그 밖에 아무 단어나 칩이 돼요. 같은 단어는 어느 문서에서나 같은 색이라 여러 글에 걸쳐 일관되게 보여요. 색이 마음에 안 들면 리더에서 바꿀 수 있어요. 단, 의미 라벨 4종(사실·추정·의견·모름)은 고정이에요.
